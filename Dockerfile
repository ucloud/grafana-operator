...
ARG PROJECT_NAME=grafana-operator
...
ARG PROJECT_NAME=grafana-operator
ARG GRAFONNET_VERSION=0.1.0

RUN apk --no-cache add curl \
    && mkdir -p /tmp/graf \
    && mkdir -p /opt/jsonnet \
    && curl -SL https://github.com/grafana/grafonnet-lib/archive/v${GRAFONNET_VERSION}.tar.gz \
    | tar -xzC /tmp/graf \
    && cp -r /tmp/graf/grafonnet-lib-${GRAFONNET_VERSION}/grafonnet /opt/jsonnet/grafonnet
